...
SdlRenderer::~SdlRenderer()
{
	...
	if (curr_framebuf.surface) {
		SDL_FreeSurface(curr_framebuf.surface);
		curr_framebuf = {};
	}
	if (last_framebuf.surface) {
		SDL_FreeSurface(last_framebuf.surface);
		last_framebuf = {};
	}
	if (window) {
		SDL_DestroyWindow(window);
                                          ...
                                          const int render_height_px)
{
	...
	curr_framebuf.surface = SDL_CreateRGBSurfaceWithFormat(Flags,
	                                                       render_width_px,
	                                                       render_height_px,
	                                                       BitDepth,
	                                                       SdlPixelFormat);

	last_framebuf.surface = SDL_CreateRGBSurfaceWithFormat(Flags,
	                                                       render_width_px,
	                                                       render_height_px,
	                                                       BitDepth,
	                                                       SdlPixelFormat);
...
void SdlRenderer::StartFrame(uint32_t*& pixels_out, int& pitch_out)
{
	...
	pixels_out = reinterpret_cast<uint32_t*>(curr_framebuf.surface->pixels);
